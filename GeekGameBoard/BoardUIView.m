...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    // Added by me (Jason L Perry) to get clicks on undraggable bits.
    else {
      if ([_game respondsToSelector:@selector(clickedUndraggableBit:)]) {
        Bit *bit = (Bit*) [self hitTestPoint: _dragStartPos
                            forLayerMatching: layerIsBit 
                                      offset: &_dragOffset];
        [_game performSelector:@selector(clickedUndraggableBit:)
                    withObject: bit];        
      }
    }  
}
